...
template = "${tab == "zsh" ? workspace : "${workspace} › ${tab}"}"
...
tabs {
  ...
  # Claude Code title "<cwd> · <name> · <session id>" -> "<name>".
  substitute {
    pattern = "^[^·]+ · (.*) · [0-9a-f-]+$"
    replace = "$1"
  }

  # oh-my-posh shell prompt title "zsh in <dir>" -> "zsh" (window template maps it to the workspace).
  substitute {
    pattern = "^zsh in .*$"
    replace = "zsh"
  }
